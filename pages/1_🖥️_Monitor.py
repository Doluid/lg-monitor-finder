...
def main():
    ...
    if st.session_state.step > 1 and st.session_state.step != 4:
        if st.button("⬅️ Zurück (이전)", key="back_btn"):
            ...
            elif st.session_state.step == 2: st.session_state.step = 1.1 if "게임" in st.session_state.data.get('purpose', '') else 1
    ...
    with col_q:
        if st.session_state.step == 1:
            ...
            if st.button("🎮 짜릿한 게임 플레이 (Gaming)"): st.session_state.data['purpose'] = "게임"; st.session_state.step = 1.1; st.rerun()
            if st.button("🎨 정밀한 사진/영상 작업 (Pro-Work)"): st.session_state.data['purpose'] = "작업"; st.session_state.step = 2; st.rerun()
            if st.button("📝 전천후 홈오피스와 멀티태스킹 (Home Office)"): st.session_state.data['purpose'] = "오피스"; st.session_state.step = 2; st.rerun()

        elif st.session_state.step == 1.1:
            st.subheader("1-1. 최고의 몰입을 선사하는 게임 스타일은?")
            if st.button("🔫 0.1초 반응이 생명인 경쟁형 FPS (예: 발로란트)"): st.session_state.data['game_style'] = "FPS"; st.session_state.step = 2; st.rerun()
            if st.button("🌍 현실보다 더 진짜 같은 오픈월드 대작"): st.session_state.data['game_style'] = "오픈월드"; st.session_state.step = 2; st.rerun()

        elif st.session_state.step == 2:
            st.subheader("2. 책상 위, 모니터를 위한 공간은 어느 정도인가요?")
            if st.button("🖥️ 24~27인치 (컴팩트 사이즈)"): st.session_state.data['size'] = "27"; st.session_state.step = 3; st.rerun()
            if st.button("📺 32~39인치 (큼직한 대화면)"): st.session_state.data['size'] = "32"; st.session_state.step = 3; st.rerun()
            if st.button("↔️ 40인치 이상 / 울트라와이드"): st.session_state.data['size'] = "40+"; st.session_state.step = 3; st.rerun()

        elif st.session_state.step == 3:
            st.subheader("3. 일상을 완벽하게 만들어줄 '단 하나의 마법'은?")
            if st.button("🔋 깔끔한 충전과 연결 (USB-C / Thunderbolt)"): st.session_state.data['feature'] = "Type-C"; st.session_state.step = 4; st.rerun()
            if st.button("🦾 내 자세에 맞춰 움직이는 자유 (Ergo)"): st.session_state.data['feature'] = "에르고"; st.session_state.step = 4; st.rerun()
            if st.button("🛞 공간 제약 없이 (거실/부엌) 이동하면서 (StandbyMe)"): st.session_state.data['feature'] = "스탠바이미"; st.session_state.step = 4; st.rerun()
    ...
    if st.session_state.step == 4:
        ...
        def create_lg_card_html(tags, title, model, rating, save_text, price, old_price, btn1_text, btn2_text, img_url, is_primary=False):
            tag_html = "".join([f"<span class='lg-tag lg-tag-{'red' if 'WM' in t or 'Vorbestellung' in t else 'outline'}'>{t}</span>" for t in tags])
        ...
        if purpose == "게임":
            cards = [
                # 52G930B-B
                create_lg_card_html(
                    ["2% mit PayPal"], "52 Zoll UltraGear evo G9, Weltweit größter 5K2K 240 Hz Gaming...", "52G930B-B", "7", "Spare 0,00 €", "1.799,00 €", "1.799,00 €", "Weitere<br>Informationen", "Info wenn<br>verfügbar",
                ...
                # 39GX950B-B
                create_lg_card_html(
                    ["Vorbestellung", "5 Jahre Garantie"], "39-Zoll-UltraGear evo GX9, der weltweit erste 39-Zoll-5K2K-OLED...", "39GX950B-B", "3", "Spare 0,01 €", "1.798,99 €", "1.799,00 €", "Weitere<br>Informationen", "Vorbestellung",
                    "https://www.lg.com/content/dam/channel/wcms/de/_it/galleries/39gx950b-b/gallery-cards-de/01_39GX950B.jpg/jcr:content/renditions/thum-350x350.jpeg", is_primary=True
                ),
                # 32GS95UE
                create_lg_card_html(
                    ["50€ WM-Bonus", "2% mit PayPal"], "LG UltraGear™ 32 Zoll 4K OLED 240Hz Gaming Monitor", "32GS95UE", "45", "Spare 200,00 €", "1.299,00 €", "1.499,00 €", "Weitere<br>Informationen", "Jetzt kaufen",
            ...
            cards = [
                # 32U990A-S
                create_lg_card_html(
                    ["Empfehlungen", "2% mit PayPal"], "LG UltraFine™ evo 32 Zoll 6K Nano IPS Black mit Thunderbolt™ 5", "32U990A-S", "80", "Spare 300,00 €", "1.699,00 €", "1.999,00 €", "Weitere<br>Informationen", "Jetzt kaufen",
                    "https://www.lg.com/content/dam/channel/wcms/de/32u990a-s/LG-IT_PRJ_Award-Gallery-Images-32U990A-S_20260504_01_450%20x%20450.jpg/jcr:content/renditions/thum-350x350.jpeg", is_primary=True
                ),
                # 32UN880-B
                create_lg_card_html(
                    ["Neu", "Ergo Stand"], "LG 32 Zoll 4K UHD Ergo Monitor mit USB-C", "32UN880-B", "120", "Spare 150,00 €", "549,00 €", "699,00 €", "Weitere<br>Informationen", "Jetzt kaufen",
                ...
                # 27ART10AKPL (스탠바이미)
                create_lg_card_html(
                    ["Lifestyle", "Smart"], "LG StanbyME 27 Zoll FHD IPS Touch Screen", "27ART10AKPL", "34", "Spare 100,00 €", "999,00 €", "1.099,00 €", "Weitere<br>Informationen", "Jetzt kaufen",
